...
function renderBriefing(){
  ...
  document.getElementById('briefing-info').innerHTML = `
    <div class="info-card bx--tile"><div class="label">Ronde</div><div class="value">${r.id} / 3</div><div class="unit">${r.name}</div></div>
    <div class="info-card bx--tile"><div class="label">Anggaran</div><div class="value text-primary">${r.budget.toLocaleString()}</div><div class="unit">Juta IDR</div></div>
    <div class="info-card bx--tile"><div class="label">Jumlah Cabor</div><div class="value">${r.minCabor}–${r.maxCabor}</div><div class="unit">Cabang olahraga</div></div>
    <div class="info-card bx--tile"><div class="label">Target Profit</div><div class="value ${r.minProfit?'text-warn':'text-accent'}">${r.minProfit?r.minProfit+'+':'Positif'}</div><div class="unit">${r.minProfit?'Minimum (M)':'Break even'}</div></div>
  `;

  document.getElementById('briefing-objectives').innerHTML = `
    <h3 class="section-title" style="color:var(--warn)">${r.name}</h3>
    <p style="color:var(--muted);font-size:13px;margin:8px 0 16px">${r.desc}</p>
    ${r.obj.map((o,i)=>`<div class="obj-item"><div class="obj-icon" style="background:rgba(69,137,255,.1);color:var(--primary)">${i+1}</div>${o}</div>`).join('')}
  `;

  document.getElementById('round-history').innerHTML = C.rounds.map((rd,i)=>{
    const h = G.history[i];
    const cls = h?'done':(i===G.round?'active':'');
    return `<div class="round-card bx--tile ${cls}">
...
async function runSimulation(){
  ...
  container.innerHTML = steps.map((_,i)=>`
    <div class="sim-step bx--tile" id="sim-step-${i}">
...
function showResults(){
  ...
  document.getElementById('score-cards').innerHTML = `
    <div class="score-card bx--tile ${sc.profitScore>=60?'good':sc.profitScore>=30?'warn':'bad'}">
    ...
    <div class="score-card bx--tile ${sc.complianceScore>=80?'good':sc.complianceScore>=50?'warn':'bad'}">
    ...
    <div class="score-card bx--tile ${sc.effScore>=60?'good':sc.effScore>=30?'warn':'bad'}">
    ...
    <div class="score-card bx--tile ${sc.riskScore>=60?'good':sc.riskScore>=30?'warn':'bad'}">
    ...
    <div class="score-card bx--tile ${sc.total>=70?'good':sc.total>=40?'warn':'bad'}" style="border-top-color:var(--primary);border-top-width:3px">
